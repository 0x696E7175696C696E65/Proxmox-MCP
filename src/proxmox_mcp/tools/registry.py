...
from proxmox_mcp.reliability import CircuitOpenError, request_fingerprint
    ...
    async def execute(
        ...
        try:
            ...
            return ToolErrorResponse(
                ...
                error=ToolError(
                    code="CIRCUIT_OPEN",
                    message=str(exc) or "Proxmox API circuit breaker is open",
    ...
    def _build_fastmcp_handler(
        ...
        async def registered_tool(
            **kwargs: object,
            ...
            # FastMCP tool surface omits actor; bind it from the authenticated
            # session so RBAC/audit identity matches the bearer token.
