...
class RuntimeBundle:
    ...
    admin_state: AdminAppState | None

    def close(self) -> None:
        """Dispose DB pool connections on process shutdown."""
        self.engine.sync_engine.dispose()


def build_runtime(settings: Settings) -> RuntimeBundle:
    bundle = asyncio.run(build_runtime_async(settings))
    # asyncio.run() closes its loop; any pooled asyncpg connections are then
    # invalid under uvicorn. Drop the pool so request handlers open fresh ones.
    bundle.close()
    return bundle
...
async def build_runtime_async(settings: Settings) -> RuntimeBundle:
    ...
    base_audit_writer: AuditWriter = PublishingAuditWriter(
    ...
    # Reconcile catalog active host before building the Proxmox client.
    # Docker process restart keeps compose-time env; hosts.local.json is the
    # durable source of truth after a WebUI switch.
    spa_path = Path(settings.admin_spa_dir)
    ...
    host_catalog = HostCatalogStore(
        path=hosts_path,
        settings=settings,
        secrets_loader=_host_secrets_loader,
        config_applier=config_store.apply_config,
    )
    host_catalog.seed_from_settings_if_empty()
    if host_catalog.reconcile_active_into_runtime():
        settings = Settings()
        config_store = ConfigStore(settings)
        host_catalog = HostCatalogStore(
            path=hosts_path,
    ...
    dependency_checkers["postgresql"] = DatabaseDependencyChecker(engine)
    ...
    identity_provider = LocalPasswordProvider(session_factory)
    if settings.admin_username and settings.admin_password is not None:
        await identity_provider.ensure_bootstrap_user(
