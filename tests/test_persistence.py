...
from proxmox_mcp.persistence.redis import build_redis_client, build_sync_redis_client
...
def test_sync_redis_client_uses_configured_url() -> None:
    settings = Settings(redis_url=SecretStr("rediss://redis.example:6379/5"))

    client = build_sync_redis_client(settings)
    pool = client.connection_pool
    raw_kwargs = pool.connection_kwargs  # pyright: ignore
    connection_kwargs = cast("dict[str, object]", raw_kwargs)
