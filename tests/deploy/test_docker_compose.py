...
def test_homelab_compose_overlay_configures_service_token_runtime() -> None:
    ...
    assert environment["PROXMOX_MCP_ADMIN_SPA_DIR"] == "/app/web/dist"
    assert "PROXMOX_MCP_ADMIN_USERNAME" in environment
    assert "PROXMOX_MCP_ADMIN_PASSWORD" in environment
    assert app["command"] == [
        "/bin/sh",
        "-c",
        "proxmox-mcp migrate && proxmox-mcp serve --mode homelab",
    ]
    assert "./secrets.local.json:/run/proxmox-mcp/secrets/secrets.json:rw" in volumes
    assert "./.env:/app/.env:rw" in volumes
