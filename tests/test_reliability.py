...
async def test_database_idempotency_store_allows_retry_after_failed_complete(
    tmp_path: Path,
) -> None:
    engine = create_async_engine(f"sqlite+aiosqlite:///{(tmp_path / 'idem-retry.db').as_posix()}")
    async with engine.begin() as connection:
        await connection.run_sync(Base.metadata.create_all)
    store = DatabaseIdempotencyStore(build_session_factory(engine))
    fingerprint = request_fingerprint({"tool": "create_vm", "parameters": {"vmid": 101}})
